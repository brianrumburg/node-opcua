...
function process_request_callback(request_data, err, response) {
    ...
    if (response && response instanceof ServiceFault) {
        console.log('ServiceFault', response);
        response.responseHeader.stringTable = response.responseHeader.stringTable ? [response.responseHeader.stringTable.join("\n")] : [];
...
var ClientSecureChannelLayer = function (options) {
    ...
    self.messageBuilder
        ...
        .on("start_chunk", function () {
            ...
            //
            console.log("request id = ", requestId,err);
            var request_data = self._request_data[requestId];
...
function _cancel_pending_transactions(err) {
    ...
    Object.keys(self._request_data).forEach(function (key) {
        ...
        //xx console.log("xxxx Cancelling pending transaction ",
        // request_data.key,request_data.msgType,request_data.request._schema.name);
